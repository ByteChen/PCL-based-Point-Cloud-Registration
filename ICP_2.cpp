...
//简单类型定义
typedef pcl::PointXYZRGBA PointT;                            //xie dai color
...
void pairAlign (const PointCloud::Ptr cloud_src, const PointCloud::Ptr cloud_tgt, PointCloud::Ptr output, Eigen::Matrix4f &final_transform, bool downsample = false)
{
  ...
  if (downsample)
  {
    grid.setLeafSize (0.002, 0.002, 0.002);      //0.05, 0.05, 0.05
    grid.setInputCloud (cloud_src);
    grid.filter (*src);
    grid.setInputCloud (cloud_tgt);
    grid.filter (*tgt);
	cout<<"src points after downsampled:"<<src->points.size()<<endl;
  }
  else
  {
    src = cloud_src;
  ...
  reg.setTransformationEpsilon (1e-15);    //1e-6
  //将两个对应关系之间的(src<->tgt)最大距离设置为10厘米
  //注意：根据你的数据集大小来调整
  reg.setMaxCorrespondenceDistance (0.01);  //0.1
  //设置点表示
  ...
  for (int i = 0; i < 100; ++i)
  {
    ...
    if (fabs ((reg.getLastIncrementalTransformation () - prev).sum ()) < reg.getTransformationEpsilon ())
     { reg.setMaxCorrespondenceDistance (reg.getMaxCorrespondenceDistance () - 0.001);      //0.001                         /// 需要调整
	   printf("缩小距离\n");
	}
...
//  *output += *cloud_src;
	...
	for (int i=0;i<16;i++) fout<<final_transform(i)<<" ";
 }
/* ---[ */
...
//    GlobalTransform = pairTransform * GlobalTransform;
    ...
    std::stringstream ss;
    ss << i << ".pcd";
    pcl::io::savePCDFile (ss.str (), *temp, false);
  }
	system("pause");
}
/* ]--- */
